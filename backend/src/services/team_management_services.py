...
class TeamManagementService():
    def add_new_team(self, new_team, captain_id):
        user_db = repository.get_user_by_id(captain_id)

        if not user_db:
            return make_http_error(404, "пользователь не найден")

        if user_db.role != "TEAM_CAPTAIN":
            return make_http_error(403, "пользователь не капитан")
        ...
        res = repository.add_new_team(new_team, captain_id, event_id)
